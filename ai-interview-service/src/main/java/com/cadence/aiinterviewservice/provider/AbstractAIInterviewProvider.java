...
public abstract class AbstractAIInterviewProvider implements AIInterviewProvider {
    ...
    private static final String EVALUATION_PROMPT_TEMPLATE = """
...

            {
              ...
              "eyeContactScore": integer 0-100 (estimate from transcript only if a video/voice cue is present, else a neutral 70),
              "speakingPaceScore": integer 0-100,
              "fillerWordCount": integer >= 0,
              "avgResponseLatencySeconds": integer >= 0 (estimate conservatively if not measurable, e.g. 5),
              "strengths": [string, ...],
              "weaknesses": [string, ...],
